...
export async function POST(req) {
  try {
    ...
    const transactionResult = await client.query(
      'INSERT INTO transactions (supplier, products, address, note, freeTax, type, status, handled_by_id, created_at, updated_at) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, NOW(), NOW()) RETURNING *',
      [
        supplier,
    ...
    // Update quantity of products (reduce for export)
    for (const product of productsDetail) {
      const productSelected = products.find((item) => item.product === product.id);
      await client.query(
        'UPDATE products SET stock = jsonb_set(jsonb_set(jsonb_set(stock, $1, to_jsonb((stock->>$2)::integer - $3)), $4, to_jsonb((stock->>$5)::integer - $6)), $7, to_jsonb((stock->>$8)::integer + $9)) WHERE id = $10',
        [
          '{total}',
          'total',
